...
function PublicBoard({ inviteUserId, onSignUp }) {
  ...
  const [activeVouchBoard, setActiveVouchBoard] = useState(null);
    ...
    const load = async () => {
      ...
      try {
        ...
        const { data: activeBoard } = await supabase
          ...
          .eq("is_active", true)
          .maybeSingle();
        setActiveVouchBoard(activeBoard || null);
  ...
  // Build VouchSection board from active vouch board items
  const vouchSectionBoard = (() => {
    const b = { movies: [], albums: [], artists: [], songs: [], books: [], shows: [] };
    if (activeVouchBoard?.vouch_board_items) {
      activeVouchBoard.vouch_board_items
  ...
  // Fix theme display: if name === theme, only show name once
  const boardTitle = activeVouchBoard?.name || "";
  const boardTheme = (activeVouchBoard?.theme && activeVouchBoard.theme !== "Other" && activeVouchBoard.theme !== boardTitle)
    ? activeVouchBoard.theme : "";
    ...
    <>
      ...
      <div className="app">
        ...
        <main className="page">
          ...
          {/* Active Vouch Board — same slideshow as signed-in view */}
          {activeVouchBoard ? (
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {boardTitle && (
                    <div style={{ fontFamily: "'Spectral',serif", fontStyle: "italic", fontSize: 12, color: "rgba(200,194,180,0.6)", marginTop: 2 }}>
                      {boardTitle}{boardTheme ? ` · ${boardTheme}` : ""}
                    </div>
                  )}
                  {activeVouchBoard.description && (
                    <div style={{ fontFamily: "'Spectral',serif", fontStyle: "italic", fontSize: 10, color: "rgba(200,194,180,0.4)", marginTop: 2 }}>{activeVouchBoard.description}</div>
                  )}
                  {activeVouchBoard.published_at && (
                    <div style={{ fontFamily: "'Spectral SC',serif", fontSize: "7px", letterSpacing: "0.1em", color: "rgba(200,194,180,0.3)", marginTop: 4 }}>
                      Published {new Date(activeVouchBoard.published_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
                    </div>
                  )}
                </div>
              </div>
          ...
          <div style={{ marginBottom: 48, textAlign: "center" }}>
            ...
            <div style={{ fontFamily: "'Spectral',serif", fontStyle: "italic", fontSize: 11, color: T.inkFaint, marginTop: 8 }}>
              Publish a Vouch Board, build your shelf, connect with friends.
            </div>
          </div>

          {/* Also on Vouch — bottom of page */}
  ...
  return (
    <div style={{ fontFamily: "'Spectral',serif", color: T.inkMid, maxWidth: 380, margin: "0 auto", textAlign: "center" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        <div>
          <div style={{ fontFamily: "'Spectral SC',serif", fontWeight: 600, fontSize: 10, letterSpacing: "0.15em", color: T.ink, marginBottom: 5 }}>Vouch Board</div>
          <div style={{ fontSize: 13, lineHeight: 1.7, fontStyle: "italic", color: T.inkMid }}>Pick up to five things you'd put your name behind right now — a film, an album, a book, a show. Publish a new one every week.</div>
        </div>
        ...
        <div>
          ...
          <div style={{ fontSize: 13, lineHeight: 1.7, fontStyle: "italic", color: T.inkMid }}>Build a permanent shelf of everything worth recommending — films, albums, books, shows, artists, songs. Your full taste in one place.</div>
        </div>
        <div style={{ borderTop: `1px solid ${T.paperDark}` }} />
        <div>
