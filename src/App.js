...
function activityBtnStyle(extra = {}) {
  return {
    background: T.ink,
    border: "none",
    color: T.bg,
...
function activityAgreePillStyle(agreed = false, extra = {}) {
  return activityBtnStyle({
    color: agreed ? AGREE_CHECK_GOLD : T.bg,
    ...extra,
  });
}

function activityQueuePillStyle(queued = false, extra = {}) {
  return {
    background: queued ? AGREE_CHECK_GOLD : T.ink,
    border: "none",
    color: queued ? T.ink : T.bg,
    cursor: "pointer",
    fontWeight: 700,
    fontFamily: "'Spectral SC',serif",
    ...extra,
...
const Styles = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .masthead-tagline { text-align: center; font-family: 'Spectral', serif; font-style: italic; font-weight: 300; font-size: 12.5px; letter-spacing: 0.12em; color: ${T.inkLight}; padding-bottom: 20px; }
    ...
    <>
      ...
      <div className="app">
        ...
        <header className="masthead">
          ...
          <div className="masthead-nameplate"><img src="/vouch5-logo.png" alt="Vouch5" style={{height:"clamp(80px,16vw,160px)",display:"block",margin:"0 auto"}} /></div>
        ...
        {showSignupNudge && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(17,16,8,0.82)", zIndex: 900, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }} onClick={() => setShowSignupNudge(false)}>
            <div style={{ background: T.bg, maxWidth: 420, width: "100%", border: `2px solid ${T.ink}`, padding: "32px 28px" }} onClick={e => e.stopPropagation()}>
              <div style={{ fontFamily: "'Times New Roman',Times,serif", fontWeight: 900, fontSize: 36, marginBottom: 4 }}><em style={{fontStyle:"italic"}}>V</em>ouch<sup style={{fontSize:"0.42em",verticalAlign:"super",lineHeight:0,fontWeight:900}}>5</sup></div>
      ...
      {visibleCats.map(cat => (
        <div key={cat.key} style={{ marginBottom: 32 }}>
          ...
          <div style={{ display: "flex", gap: 12, overflowX: "auto", paddingBottom: 4, scrollbarWidth: "none" }}>
            {catItems[cat.key].slice(0, 5).map((item, i) => <TileCard key={i} item={item} catKey={cat.key} />)}
          </div>
        </div>
      ))}
          ...
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxHeight: "88vh", display: "flex", flexDirection: "column" }}>
            ...
            <div className="modal-body" style={{ overflowY: "auto", flex: 1 }}>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 14 }}>
                {catItems[modalCat].map((item, i) => <TileCard key={i} item={item} catKey={modalCat} />)}
              </div>
            </div>
          </div>
        </div>
        ...
        <div key={`${isDiscovery ? "d" : "b"}-shelf-${i}`} style={{ borderBottom: "1px solid #b3ada0", paddingBottom: 16, marginBottom: 16, opacity: isDiscovery ? 0.92 : 1 }}>
          ...
          <div style={{ width: "100%", maxWidth: 300, margin: "0 auto", position: "relative" }}>
            ...
            {onDudeSame && buddy && buddy.userId !== selfId && (
              <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
                <button onClick={e => { e.stopPropagation(); onDudeSame({ id: primary.item_id, title: primary.title, poster: primary.poster, _cat: primary.category }, buddy.userId); }} style={activityAgreePillStyle(!!(myReactions||[]).find(r => r.item_id === String(primary.item_id) && r.item_owner_id === buddy.userId), { flex: 1, fontSize: "8px", letterSpacing: "0.1em", padding: "6px 4px" })}>{(myReactions||[]).find(r => r.item_id === String(primary.item_id) && r.item_owner_id === buddy.userId) ? "✓ Agreed" : "Agree"}</button>
                {onAddToQueue && <button onClick={e => { e.stopPropagation(); onAddToQueue({ id: primary.item_id, title: primary.title, poster: primary.poster, source_url: primary.source_url, category: primary.category, user_id: buddy.userId }); }} style={activityQueuePillStyle(!!(queue||[]).find(q => String(q.id) === String(primary.item_id)), { flex: 1, fontSize: "8px", letterSpacing: "0.1em", padding: "6px 4px" })}>{(queue||[]).find(q => String(q.id) === String(primary.item_id)) ? "✓ Queued" : "+ Queue"}</button>}
              </div>
            )}
          </div>
        </div>
      ...
      return (
        <div key={`${isDiscovery ? "d" : "b"}-comment-${i}`} style={{ borderBottom: "1px solid #b3ada0", paddingBottom: 24, marginBottom: 24, opacity: isDiscovery ? 0.92 : 1 }}>
          ...
          <div style={{ width: "100%", maxWidth: 300, margin: "0 auto" }}>
            ...
            <button type="button" onClick={goToVouch} style={activityBtnStyle({ marginTop: 10, width: "100%", fontSize: "8px", letterSpacing: "0.12em", padding: "7px 10px" })}>View Vouch</button>
          </div>
        </div>
      );
    }
          ...
          {r.poster && (
            <div style={{ width: "100%", maxWidth: 300, margin: "0 auto", position: "relative" }}>
              ...
              {onDudeSame && r.item_owner_id && r.item_owner_id !== selfId && (
                <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
                  <button onClick={() => onDudeSame({ id: r.item_id, title: r.title, poster: r.poster, _cat: r.category }, r.item_owner_id)} style={activityAgreePillStyle(!!(myReactions||[]).find(x => x.item_id === r.item_id && x.item_owner_id === r.item_owner_id), { flex: 1, fontSize: "8px", letterSpacing: "0.1em", padding: "6px 4px" })}>
                    {(myReactions||[]).find(x => x.item_id === r.item_id && x.item_owner_id === r.item_owner_id) ? "✓ Agreed" : "Agree"}
                  </button>
                  {onAddToQueue && <button onClick={() => onAddToQueue({ id: r.item_id, title: r.title, poster: r.poster, source_url: r.source_url, category: r.category })} style={activityQueuePillStyle(!!(queue||[]).find(q => q.id === r.item_id), { flex: 1, fontSize: "8px", letterSpacing: "0.1em", padding: "6px 4px" })}>
...
function StartPage({ onSignUp }) {
    ...
    const load = async () => {
      try {
        const [movieRes, albumRes] = await Promise.all([
          fetch(`https://api.themoviedb.org/3/movie/popular?api_key=24f3b03466f2f7db2d54a0f53607da4f&language=en-US&page=1`).then(r => r.json()),
          fetch(`/api/spotify?q=best+albums+2024&type=album`).then(r => r.json()),
        ]);
  ...
  const dummyVouchBoard = {
    movies: featuredMovies.slice(0, 3).map(m => ({ ...m, vouched: true, _cat: "movies", _catLabel: "Film" })),
    albums: featuredAlbums.slice(0, 2).map(a => ({ ...a, vouched: true, _cat: "albums", _catLabel: "Albums" })),
  ...
  const generateShareCard = async (board = activeBoard) => {
    ...
    const drawCard = async (posterImgs) => {
      ...
      if (vLogo.naturalWidth > 0) {
        const logoW = vLogo.naturalWidth * (logoH / vLogo.naturalHeight);
        ctx.drawImage(vLogo, (W - logoW) / 2, 60, logoW, logoH);
      }

      // Double rule
      ctx.strokeStyle = "#111008"; ctx.lineWidth = 4;
      ctx.beginPath(); ctx.moveTo(56, 290); ctx.lineTo(W-56, 290); ctx.stroke();
      ctx.lineWidth = 1.5;
      ctx.beginPath(); ctx.moveTo(56, 300); ctx.lineTo(W-56, 300); ctx.stroke();

      // Theme name
      ctx.fillStyle = "#111008"; ctx.font = "900 64px 'Times New Roman', serif"; ctx.textAlign = "center";
      ctx.fillText(boardTheme.length > 22 ? boardTheme.slice(0,22)+"…" : boardTheme, 540, 372);
      ctx.fillStyle = "#7a7568"; ctx.font = "italic 400 30px Georgia";
      ...
      const dl = (item, cx, y) => {
        if (!item) return;
        const cat = catLabels[item.category] || "";
        const yr = item.subtitle ? " (" + item.subtitle.slice(0,4) + ")" : "";
        const ttl = ((item.title||"").slice(0,16) + ((item.title||"").length>16?"…":"")) + yr;
        ctx.fillStyle = "#111008"; ctx.font = "600 20px 'Times New Roman', serif"; ctx.textAlign = "center";
        ctx.fillText(ttl, cx, y);
        ctx.fillStyle = "rgba(17,16,8,0.4)"; ctx.font = "400 16px Georgia";
        ctx.fillText(cat.toUpperCase(), cx, y+22);
        ctx.textAlign = "left";
      };
      ...
      <div className="app">
        <header className="masthead">
          ...
          <div className="masthead-nameplate" onClick={() => { setTab("board"); setViewing(null); window.history.replaceState({}, "", "/"); scrollToTop(); }}>
            <img src="/vouch5-logo.png" alt="Vouch5" style={{height:"clamp(80px,16vw,160px)",display:"block",margin:"0 auto"}} />
          </div>
          <div className="masthead-rule-ornament"><span>—</span><span>✦</span><span>—</span></div>
          <div className="masthead-tagline">Love it? Vouch for it.</div>
          <nav className="nav">
