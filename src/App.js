              ...
              <div style={{ width: "100%", maxWidth: 300, margin: "0 auto" }}>
                ...
                {onDudeSame && buddy && buddy.userId !== selfId && (
                  <div style={{ display: "flex", marginTop: 8 }}>
                    ...
                    {onAddToQueue && <button onClick={() => onAddToQueue({ id: primary.item_id || primary.id, title: primary.title, poster: primary.poster, source_url: primary.source_url, category: primary.category, user_id: buddy.userId })} style={{ flex: 1, background: (queue||[]).find(q => q.id === String(primary.item_id || primary.id)) ? "rgba(17,16,8,0.15)" : "transparent", border: "1px solid #b3ada0", borderLeft: "none", color: "#3a3830", cursor: "pointer", fontSize: "8px", fontFamily: "'Spectral SC',serif", letterSpacing: "0.1em", padding: "6px 4px", fontWeight: 700 }}>{(queue||[]).find(q => q.id === String(primary.item_id || primary.id)) ? "✓ Queued" : "+ Queue"}</button>}
              </div>
                )}
              </div>
          );
